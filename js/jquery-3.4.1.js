...
var documentElement = document.documentElement;
	...
	// Verifique el adjunto a través de los límites del DOM de sombra cuando sea posible (gh-3504)
	// Soporte: solo iOS 10.0-10.2
	// Las versiones anteriores de iOS 10 admiten `attachShadow` pero no` getRootNode`,
	// conduciendo a errores. Necesitamos verificar `getRootNode`.
...
función adjustCSS (elem, prop, valueParts, tween) {
	...
	if (initialInUnit && initialInUnit [3]! == unidad) {
		...
		while (maxIterations--) {
			...
			// Evalúe y actualice nuestras mejores conjeturas (duplicando las conjeturas que se ponen a cero).
...
Función buildFragment (elementos, contexto, scripts, selección, ignorado) {
	var elem, tmp, etiqueta, envolver, adjuntado, j,
...
// Soporte: IE <= 9 - 11+
// focus () y blur () son asíncronos, excepto cuando no son opcionales.
// Así que espere que el foco esté sincronizado cuando el elemento ya esté activo,
// y desenfoque para ser sincrónico cuando el elemento aún no está activo.
// (el foco y el desenfoque son siempre sincrónicos en otros navegadores compatibles,
// esto solo define cuando podemos contar con ello).
función expectSync (elem, tipo) {
...
jQuery.event = {
	...
	dispatch: function (nativeEvent) {
		...
		while ((matched = handlerQueue [i ++]) &&! event.isPropagationStopped ()) {
			...
			while ((handleObj = matched.handlers [j ++]) &&
				...
				// Si el evento tiene espacio de nombre, entonces cada controlador solo se invoca si es
				// especialmente los universales o sus espacios de nombres son un superconjunto del evento.
	...
	especial: {
		...
		haga clic en: {
			...
			// Utilizar el evento nativo para asegurar el estado correcto para las entradas verificables
			configuración: función (datos) {

				// Para la compresibilidad mutua con _default, reemplace el acceso `this` con una var local.
				// `|| data` es un código muerto destinado únicamente a preservar la variable a través de la minificación.
				var el = este || datos;
				...
				// Devuelve false para permitir el procesamiento normal en la persona que llama
				falso retorno;
			}
			...
			// También impídalo si estamos actualmente dentro de una pila de eventos nativos apalancados
			_default: función (evento) {
				var target = event.target;
				devolver rcheckableType.test (target.type) &&
...
// Asegurar la presencia de un detector de eventos que se maneja manualmente
// eventos sintéticos al interrumpir el progreso hasta que se reinicie en respuesta a
// * nativo * eventos que se activan directamente, asegurando que los cambios de estado tengan
// ya ocurrió antes de invocar a otros oyentes.
apalancamiento de funciónNative (el, type, expectSync) {

	// Falta expectSync indica una llamada de activación, que debe forzar la configuración a través de jQuery.event.add
	if (! expectSync) {
	...
	jQuery.event.add (el, tipo, {
		...
		manejador: función (evento) {
			...
			if ((event.isTrigger & 1) && este [tipo]) {
				...
				// Interrumpir el procesamiento del evento .trigger () ed sintético externo
				// Los datos guardados deben ser falsos en tales casos, pero podría ser un objeto de captura sobrante
				// desde un controlador nativo asíncrono (gh-4350)
				if (! saved.length) {

					// Almacenar argumentos para usar al manejar el evento nativo interno
					...
					// Soporte: IE <= 9 - 11+
					// focus () y blur () son asíncronos
					notAsync = expectSync (este, tipo);
					este tipo ]();
					...
					si (guardado! == resultado) {
						...
						// Cancelar el evento sintético exterior
						event.stopImmediatePropagation ();
						event.preventDefault ();
						devuelve result.value;
					}

				// Si este es un evento sintético interno para un evento con un sustituto burbujeante
				// (enfoque o desenfoque), suponga que el sustituto ya se propagó al activar el
				// Evento nativo y evita que eso vuelva a suceder aquí.
				// Esto técnicamente hace que la ordenación incorrecta se envíe a `.trigger ()` (en la que
				// el sustituto de propagación burbujeante se propaga * después de * la base que no burbujea), pero eso parece
				// menos malo que la duplicación.
				} else if ((jQuery.event.special [type] || {}) .delegateType) {
					event.stopPropagation ();
				}

			// Si este es un evento nativo activado anteriormente, todo está ahora en orden
			// Dispara un evento sintético interno con los argumentos originales.
			} else if (saved.length) {

				// ... y captura el resultado
				dataPriv.set (este, escribe, {
					valor: jQuery.event.trigger (

						// Soporte: IE <= 9 - 11+
						// Amplíe con el prototipo para restablecer la anterior detención de la imágen ()
...
jQuery.each ({focus: "focusin", blur: "focusout"}, function (type, delegateType) {
	jQuery.event.special [tipo] = {
		...
		// Utilizar el evento nativo si es posible para que la secuencia de desenfoque / enfoque sea correcta
		configuración: función () {

			// Reclamar el primer controlador
			// dataPriv.set (esto, "focus", ...)
			// dataPriv.set (esto, "blur", ...)
			leverageNative (este, tipo, expectSync);
...
(función () {
	...
	función computeStyleTests () {
		...
		// Soporte: Chrome <= 64
		// No se deje engañar cuando el zoom afecta a offsetWidth (gh-4029)
...
// Devolver una propiedad prefijada por el vendedor o undefined
...
// Devolver una propiedad jQuery.cssProps o proveedor prefijada potencialmente asignada
...
function boxModelAdjustment (elem, dimension, box, isBorderBox, styles, computedVal) {
	...
	if (! isBorderBox && computedVal> = 0) {
		...
		// Si se desconoce offsetWidth / offsetHeight, no podemos determinar el contenido del cuadro de desplazamiento
...
función getWidthOrHeight (elemento, dimensión, extra) {
	...
	var styles = getStyles (elem),
		...
		// Cuadro de contenido falso hasta que sepamos que es necesario conocer el valor verdadero.
	...
	// Soporte: IE 9-11 solamente
	// También usa offsetWidth / offsetHeight para cuando el tamaño de la caja no es confiable
	// Usamos getClientRects () para verificar si está oculto / desconectado.
	// En esos casos, se puede confiar en que el valor calculado sea un cuadro de borde
	if ((! support.boxSizingReliable () && isBorderBox ||
		...
		// Donde esté disponible, offsetWidth / offsetHeight dimensiones de cuadro de borde aproximadas.
		// Cuando no esté disponible (por ejemplo, SVG), asuma un tamaño de caja no confiable e interprete el
		// valor recuperado como una dimensión de cuadro de contenido.
...
jQuery.extend ({
	...
	estilo: función (elemento, nombre, valor, extra) {
		...
		si (valor! == indefinido) {
			...
			// La comprobación de isCustomProp se puede eliminar en jQuery 4.0 cuando solo agregamos automáticamente
	...
	jQuery.cssHooks [dimensión] = {
		...
		set: function (elem, value, extra) {
			coincidencias var
				...
				// Solo lea styles.position si la prueba tiene la oportunidad de fallar
